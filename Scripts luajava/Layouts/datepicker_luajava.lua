...
local DatePicker = bind("android.widget.DatePicker")
...
local picker = DatePicker(activity)
...
activity.runOnUiThread(luajava.createProxy("java.lang.Runnable", {
    run = function()
        ...
        btn.setOnClickListener(luajava.createProxy("android.view.View$OnClickListener", {
            onClick = function(v)
                local day = picker.getDayOfMonth()
                local month = picker.getMonth() + 1
                local year = picker.getYear()

                gg.toast("Date: " .. day .. "/" .. month .. "/" .. year)
